...
namespace Huobi.Net.Clients.SpotApi
{
    ...
    public class HuobiSocketClientSpotApi : SocketApiClient, IHuobiSocketClientSpotApi
    {
        ...
        #region ctor
        internal HuobiSocketClientSpotApi(Log log, HuobiSocketClientOptions options)
        ...
        public async Task<CallResult<HuobiIncementalOrderBook>> GetOrderBookAsync(string symbol, int levels)
        {
            ...
            var result = await QueryAsync<HuobiSocketResponse<HuobiIncementalOrderBook>>(request, false).ConfigureAwait(false);
            if (!result)
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToOrderBookChangeUpdatesAsync(string symbol, int levels, Action<DataEvent<HuobiIncementalOrderBook>> onData, CancellationToken ct = default)
        {
            ...
            return await SubscribeAsync(_baseAddressMbp, request, null, false, internalHandler, ct).ConfigureAwait(false);
        }
        ...
        /// <inheritdoc />
        public async Task<CallResult<IEnumerable<WWTHuobiSpecifiedTimeKLine>>> GetSpecifiedTimeKLinesAsync(string symbol, KlineInterval period, long startTimeStamp, long endTimeStamp)
        {
            symbol = symbol.ValidateHuobiSymbol();
            ...
            CallResult<HuobiSocketResponse<IEnumerable<WWTHuobiSpecifiedTimeKLine>>>? result = await QueryAsync<HuobiSocketResponse<IEnumerable<WWTHuobiSpecifiedTimeKLine>>>(request, false).ConfigureAwait(false);
            return result ? result.As(result.Data.Data) : result.AsError<IEnumerable<WWTHuobiSpecifiedTimeKLine>>(result.Error!);
        }
